...
OAuth는 인가 계층(authorization layer)을 도입하고, 클라이언트(client)의 역할과 자원 소유자(resource owner)의 역할을 분리함으로써 이러한 문제들을 해결합니다.
...
HTTP가 아닌 다른 프로토콜에서 OAuth를 사용하는 것은 범위를 벗어납니다. 정보 문서로 발행된 OAuth 1.0 프로토콜(RFC5849)은 소규모 임시 커뮤니티 노력의 결과였습니다. 이 표준 트랙 명세서는 OAuth 1.0 배포 경험과 더 넓은 IETF 커뮤니티에서 수집된 추가 사용 사례 및 확장성 요구사항을 바탕으로 합니다. OAuth 2.0 프로토콜은 OAuth 1.0과 하위 호환되지 않습니다. 두 버전은 네트워크에서 공존할 수 있으며, 구현체는 둘 다 지원하도록 선택할 수 있습니다. 그러나 이 명세서의 의도는 새로운 구현체가 이 문서에 명시된 OAuth 2.0을 지원하고, OAuth 1.0은 기존 배포를 지원하는 용도로만 사용되는 것입니다. OAuth 2.0 프로토콜은 OAuth 1.0 프로토콜과 매우 적은 구현 세부사항을 공유합니다. OAuth 1.0에 익숙한 구현자들은 이 문서의 구조와 세부사항에 대해 어떤 가정도 하지 말고 접근해야 합니다.
